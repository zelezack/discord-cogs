    ...
    async def ships(self, ctx):
        ...
        if ctx.invoked_subcommand is None:
                for p in parsed['fleet']:
                    ...
                    if ordname == 'utcp55':
                        members = ', '.join(map(str,p['members']))
                        utcp55.append(members)
                        utcp55flag = p['flag']
                        utcp55tz = p['tzname']
                        utcp55utc = datetime.strptime(p['utctime'],'%H:%M:%S') - (datetime.strptime(current_time,'%H:%M:%S'))
                        if datetime.strptime(p['utctime'],'%H:%M:%S') < datetime.strptime(current_time,'%H:%M:%S'):
                            utcp55payout = ' :moneybag:'
                        else:
                            utcp55payout = ' - :clock130: ' + str(utcp55utc)
                    if ordname == 'utcp5':
                        members = ', '.join(map(str,p['members']))
...
#=============================================================================================================================
                    if ordname == 'utcm11':
                        ...
                        utcm11flag = p['flag']
                        utcm11tz = p['tzname']
                        utcm11utc = datetime.strptime(p['utctime'],'%H:%M:%S') - (datetime.strptime(current_time,'%H:%M:%S'))
                        if datetime.strptime(p['utctime'],'%H:%M:%S') < datetime.strptime(current_time,'%H:%M:%S'):
                ...
                await self.bot.say("Payout list:\n" +
                    ...
                    #utcp10flag + utcp10tz + " - " + ', '.join(utcp10) + str(utcp10payout) +"\n"+
                    ...
                    utcp55flag + utcp55tz + " - " + ', '.join(utcp55) + str(utcp55payout) +"\n"+
    ...
    async def _ships_add(self,ctx,timezone, *, name):
        ...
        with open('cogs/assets/ships.json', 'w') as json_file:
